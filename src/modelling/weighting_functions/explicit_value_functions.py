...
def value_funcs_inferred_individual(
    rng_key, agents, cat_phis, vself, c=0.1, alpha=10, bins=50
):
    ...
    return value_funcs_known(rng_key, agents_, cat_phis, vself)


def value_funcs_inferred_group(rng_key, agents, cat_phis, vself, c=0.1, alpha=10, bins=50):
    weights = jnp.zeros(len(cat_phis))
